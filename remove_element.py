...
class Solution:
    def removeElement(self, nums: list[int], val: int) -> int:
        ...
        underscores = 0
        while True:
            if i >= length - underscores:
                return length - underscores
            if nums[i] == val:
                if nums[length - 1 - underscores] != val:
                    nums[i], nums[length - 1 - underscores]  = nums[length - 1 - underscores], nums[i]
                    i += 1
                underscores += 1
            else:
                i += 1
...
#Tests:
testLists= [[3,2,2,3], [0,1,2,2,2,0,4,2]]
testValues = [3,2]
expectedOutputs = [2, 4]
